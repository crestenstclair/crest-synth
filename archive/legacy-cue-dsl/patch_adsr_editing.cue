...
project: requirements: {
	patch_adsr_focus_surface: {
		kind: "functional"
		description: "PATCH Main exposes one reducer-owned nonwrapping focused-Patch order whose base is Engine, Attack, Decay, Sustain, Release from PatchControlId plus the canonical VoiceEnvelope descriptor and whose suffix is the active instrument descriptor's StructuralChoice controls followed by ordered effect ScalarEdit controls; bare Up/Down moves one row, Right enters persistent Utility with an exact return path, Left on Main and endpoint navigation are typed unchanged rejections, and exactly one semantic target is focused"
	...
	canonical_patch_adsr_adjustment: {
		kind: "functional"
		description: "On a focused ADSR row, Edit+Left/Right applies the existing descriptor's fine decrement/increment and Edit+Down/Up applies its coarse decrement/increment through AppState.apply; the reducer reuses VoiceEnvelopeParameter and VoiceEnvelope.withValue and changes exactly the focused Patch field without exposing Patch envelopes as mixer-track controls"
		goals: ["goal.edit_patch_envelope"]
