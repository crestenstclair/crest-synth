...
project: {
	...
	goals: {
		...
		control_synth: {
			description: "The player can navigate and edit every current mixer, envelope, engine-scalar, and global parameter through normalized SemanticActions while the semantic graphical model, retained diagnostics, and audio projections remain downstream of the same capability-polymorphic one-way loop"
			priority: "required"
			actors: ["actor.player", "actor.maintainer"]
	...
	capabilities: {
		...
		global_mix: {
			description: "Trim and route every already post-effect-processed Patch stem into one of sixteen tracks, then apply track level, pan, mute, solo, sends, one mixer-owned global reverb, and one mixer-owned global delay"
			goals: ["goal.play_test_song", "goal.control_synth"]
			acceptance: shared_effects: {
				description: "Patch output routes, track parameters, and the two shared effects alter the final stereo signal"
				actor: "actor.maintainer"
				steps: [
					{action: "render two simultaneously active Patches on distinct tracks and then one shared track", observes: "Patch identity is retained through trim and routing; the shared-track stems sum before the one track parameter set and track sends feed the shared effects"},
					{action: "change one track level and one Patch trim", observes: "the track edit changes every contribution routed there while the Patch edit changes only that Patch and unrelated tracks remain sample-identical"},
		...
		one_way_parameter_control: {
			description: "Translate physical or passive-view input to SemanticActions and then AppEvents, reduce them into AppState, serialize accepted instrument/effect/interaction state, project semantic graphical, diagnostic, and fixed real-time values, and publish only accepted effects"
			...
			acceptance: keyboard_edit: {
				...
				actor: "actor.player"
				steps: [
					{action: "press W, S, A, or D", observes: "one SemanticAction becomes one AppEvent and the stable FocusPath moves without changing a synth parameter"},
					{action: "hold K and press a direction, then release K", observes: "mode projects Adjust while held, exactly the selected bounded parameter changes through AppState.apply, and release restores Navigate"},
		...
		schema_driven_patch_page: {
			...
			acceptance: two_context_projection: {
				...
				steps: [
					...
					{action: "send leftward PATCH Main navigation, a vertical engine adjustment, or endpoint navigation", observes: "the reducer returns the applicable typed unchanged rejection, while rightward navigation enters Utility with an exact return path and a later valid navigation, adjustment, or context-selection action remains accepted"},
				]
		...
		observable_demo_scene: {
			...
			acceptance: falsifiable_seams: {
				...
				steps: [
					...
					{action: "copy one edited track parameter set into a different published MixerTrackId", observes: "exact StateTree-to-ParameterSnapshot equality and untargeted track audio isolation fail"},
	...
	requirements: {
		...
		mixer_global_effects_only: {kind: "functional", description: "MixEngine contains exactly one shared reverb and one shared delay fed by post-gate track sends and owns no other processor; the separately prepared upstream Patch effect rack contains the one configured Chorus insert", goals: ["goal.play_test_song", "goal.shape_patch_with_effect"], capabilities: ["capability.global_mix", "capability.static_patch_effect"]}
		...
		one_way_loop: {kind: "nonfunctional", description: "Every user input becomes one SemanticAction and then one AppEvent while startup, MIDI, worker, and system sources enter as typed AppEvents; AppState.apply commits accepted state before semantic graphical/text projection, serialization, parameter publication, or audio-command effects", goals: ["goal.control_synth", "goal.use_semantic_graphical_view_model"], capabilities: ["capability.one_way_parameter_control", "capability.semantic_graphical_view_model"]}
		...
		basic_text_contexts: {kind: "functional", description: "TextProjection remains one immutable lossless diagnostic projection at a time: MIXER preserves exactly sixteen track sections plus distinct globals and routed-Patch summaries while PATCH renders the focused schema-derived Patch page and output values; the production AppWindow receives GraphicalShellProjection embedding SemanticGraphicalViewModel, while diagnostics add no third top-level context, focus authority, valid-action source, or writable UI state", goals: ["goal.control_synth", "goal.inspect_patch", "goal.use_graphical_shell", "goal.use_semantic_graphical_view_model"], capabilities: ["capability.one_way_parameter_control", "capability.schema_driven_patch_page", "capability.graphical_application_shell", "capability.semantic_graphical_view_model"]}
		keyboard_controls: {kind: "functional", description: "1 directly selects MIXER, 2 directly selects PATCH, bare W/S emits semantic vertical Navigate, bare A/D emits horizontal Navigate, K down/up emits Adjust/Navigate mode and K plus W/S/A/D emits Adjust; AppLoop maps every normalized SemanticAction to AppEvent before AppState.apply, AppState resolves behavior from FocusPath and mode, and no adapter mutates semantic state directly", goals: ["goal.control_synth", "goal.inspect_patch", "goal.edit_patch_envelope", "goal.select_soundfont_preset", "goal.use_semantic_graphical_view_model"], capabilities: ["capability.one_way_parameter_control", "capability.schema_driven_patch_page", "capability.soundfont_preset_selection", "capability.semantic_graphical_view_model"]}
		...
		semantic_context_selection: {kind: "nonfunctional", description: "Digit 1 and Digit 2 normalize to WindowInput, translate to SemanticAction::SelectContext(MIXER|PATCH), map to AppEvent, reduce only through AppState.apply, restore the destination remembered FocusPath in Navigate mode, and project from committed InteractionState; window, test, and view adapters own no context state", goals: ["goal.inspect_patch", "goal.use_semantic_graphical_view_model"], capabilities: ["capability.one_way_parameter_control", "capability.schema_driven_patch_page", "capability.semantic_graphical_view_model"]}
		...
		stable_patch_focus: {kind: "nonfunctional", description: "InteractionState owns PATCH focus as a stable PatchMain FocusPath separately from the remembered semantic MIXER path; installation initializes it deterministically to the first Patch Engine, context switches retain it, and a missing or stale target is repaired only by the reducer's deterministic descriptor sibling rule before projection, never by a view or vector index", goals: ["goal.inspect_patch", "goal.use_semantic_graphical_view_model"], capabilities: ["capability.schema_driven_patch_page", "capability.semantic_graphical_view_model"]}
		projection_only_context_switch: {kind: "nonfunctional", description: "Selecting PATCH or MIXER changes only reducer-owned interaction context and the generation-coherent serialization/view projection; Patch/config/envelope/output, all sixteen track values, globals, ParameterSnapshot values, PreparedGraph revision and ownership, MIDI routing, audio commands, and rendered behavior remain unchanged", goals: ["goal.inspect_patch"], capabilities: ["capability.one_way_parameter_control", "capability.schema_driven_patch_page", "capability.realtime_execution"]}
		...
		evolvable_boundaries: {kind: "nonfunctional", description: "Instrument and effect capability metadata, their separate off-thread preparation and capability-neutral prepared rendering boundaries, MIDI input, audio output, passive graphical rendering over the host-neutral semantic model, retained diagnostic projection, discrete commands, scalar snapshots, and structural graph ownership are expressed as replaceable ports", goals: ["goal.control_synth", "goal.shape_patch_with_effect", "goal.use_graphical_shell", "goal.use_semantic_graphical_view_model"], capabilities: ["capability.instrument_capability_model", "capability.static_patch_effect", "capability.prepared_engine_rack", "capability.one_way_parameter_control", "capability.graphical_application_shell", "capability.semantic_graphical_view_model", "capability.realtime_execution"]}
		deterministic_demo_scene: {kind: "functional", description: "A headless demo scene drives the same normalized 1/2/W/S/A/D/K input translator and production AppLoop as EframeGraphicalWindow, with deterministic checkpoints and no native window or physical audio device", goals: ["goal.observe_synth"], capabilities: ["capability.observable_demo_scene", "capability.schema_driven_patch_page", "capability.graphical_application_shell"]}
		...
		exhaustive_current_surface: {kind: "nonfunctional", description: "The demo and table-driven tests cover every declared AppEvent variant and direction, every supported MidiMessage kind, every current editable Patch parameter, Patch output field, all six parameter classes on every one of sixteen tracks, all seven global parameters, every serialized state/projection property, accepted and rejected outcomes, and measured downstream effects", goals: ["goal.observe_synth"], capabilities: ["capability.observable_demo_scene"]}
		...
		egui_context_verification: {kind: "functional", description: "A headless egui Context processes real egui key/focus events through EframeGraphicalApplication update with its callback wired to AppLoop, then proves the next frame, EventLog, accepted state, exact GraphicalShellProjection and retained TextProjection values, region geometry, and scroll target all reflect that event without opening a native window", goals: ["goal.observe_synth", "goal.use_graphical_shell"], capabilities: ["capability.observable_demo_scene", "capability.one_way_parameter_control", "capability.graphical_application_shell"]}
		seam_mutation_falsifiability: {kind: "nonfunctional", description: "Six isolated verification-only mutants—dropped adjustment, cross-track parameter leak, PatchId MIDI misroute, omitted StateTree leaf, dry-to-wet bypass, and zeroed renderer output—must each falsify its own typed witness without manufacturing coverage gaps or altering a completed report", goals: ["goal.observe_synth"], capabilities: ["capability.observable_demo_scene"]}
		separate_live_demo: {kind: "functional", description: "make demo-live-graphical-shell retains the Phase 1 optimized physical witness; make demo-live-semantic-view-model invokes the newest bounded release scene in the normal eframe window and CPAL stream, emits completed semantic/shell/lifecycle evidence, closes, and returns; make demo-live aliases that cumulative Phase 2 scene and make demo retains its exact headless command and behavior", goals: ["goal.observe_live_synth", "goal.use_graphical_shell", "goal.use_semantic_graphical_view_model"], capabilities: ["capability.live_observable_demo", "capability.graphical_application_shell", "capability.semantic_graphical_view_model"]}
		...
		live_current_surface: {kind: "functional", description: "The expected live scalar coverage set is frozen from production Patch, PatchOutput, MixerTrackParameter, and GlobalParameters descriptors plus installed Patch and fixed MixerTrack identities; every Patch-owned scalar, both output fields, all six track parameter classes across the sixteen-track bank, and every global instance is causally exercised through normal semantic paths with exact-generation audible or toggle evidence, while the separate ordered structural-transition set retains one adjacent authored-name SoundFont preset selection plus SoundFont-to-Braids and Braids-to-descriptor-default-SoundFont for the focused first Patch", goals: ["goal.observe_live_synth", "goal.select_soundfont_preset", "goal.shape_patch_with_effect"], capabilities: ["capability.live_observable_demo", "capability.global_mix", "capability.per_voice_envelope", "capability.braids_engine", "capability.static_patch_effect", "capability.asynchronous_engine_selection", "capability.soundfont_preset_selection"]}
		canonical_live_projection: {kind: "nonfunctional", description: "The visible frame, EventRecord, StateTree, SemanticGraphicalViewModel, GraphicalShellProjection, retained TextProjection, ParameterSnapshot, structural intent/status, and active graph revision at each live checkpoint all derive from the same accepted AppState generation and correlated structural status; the live runner has no UI-owned, worker-owned, graph-owned, or engine-owned state copy", goals: ["goal.observe_live_synth", "goal.select_soundfont_preset", "goal.use_graphical_shell", "goal.use_semantic_graphical_view_model"], capabilities: ["capability.live_observable_demo", "capability.one_way_parameter_control", "capability.graphical_application_shell", "capability.semantic_graphical_view_model", "capability.asynchronous_engine_selection", "capability.soundfont_preset_selection"]}
		...
		control_path: {kind: "behavioral", description: "keyboard-equivalent MIXER track and PATCH output/value edits change only their canonical selected values and routed audio contributions, focus-only navigation is audio-neutral, boundary no-ops remain nonfatal, and sustained fifteen-Patch MIDI dispatch stays within its measured responsiveness ceiling", validations: ["validation.smoke", "validation.patch_page_projection", "validation.control_dispatch_performance", "validation.sixteen_track_mixer_routing", "validation.test"], witnesses: ["witness.control_path"]}
		...
		mutation_resistance: {kind: "behavioral", description: "independent production-seam mutants for dropped adjustment, cross-track parameter leakage, Patch MIDI misrouting, StateTree leaf omission, dry-to-wet bypass, and zero renderer output are each rejected by a typed engine-executed witness", validations: ["validation.mutation_harness", "validation.test"], witnesses: ["witness.dropped_adjustment_mutant", "witness.cross_track_parameter_leak_mutant", "witness.patch_misroute_mutant", "witness.omitted_state_tree_leaf_mutant", "witness.dry_to_wet_bypass_mutant", "witness.zero_renderer_mutant"]}
		...
		braids_engine_contract: {kind: "behavioral", description: "the pinned native adapter, FixedPerPatch(16) descriptor, 16 × N scaling across N admitted Braids Patches, exact sample-rate policy, mixed routing, scalar effects, lifecycle, and timing are verified through production preparation/render seams", validations: ["validation.braids_engine", "validation.prepared_engine_rack", "validation.test"]}
			per_voice_envelope_contract: {kind: "behavioral", description: "the canonical four-field envelope projects and edits exactly through PATCH without becoming mixer-track state, then independently controls overlapping SoundFont and Braids note voices", validations: ["validation.per_voice_envelope", "validation.patch_page_projection", "validation.prepared_engine_rack", "validation.test"]}
			patch_page_contract: {kind: "behavioral", description: "the production input translator, reducer, serializer, context projector, graphical eframe adapter with retained diagnostic, and audio boundary prove exact two-context selection, descriptor-derived instrument/effect rows, dynamic Engine-plus-ADSR-plus-structural-choice-plus-effect-scalar focus, canonical ADSR/effect edits, and reducer-owned structural status", validations: ["validation.patch_page_projection", "validation.static_patch_effect", "validation.per_voice_envelope", "validation.soundfont_preset_selection", "validation.schema_surface", "validation.egui_context", "validation.engine_selection_workflow", "validation.test"]}
		}
	...
	nonGoals: {
		...
		elaborate_ui: "crest-synth now provides the Phase 1 graphical shell, Phase 2 host-neutral semantic graphical contract, and the corrective canonical sixteen-track routing/control slice; it does not yet provide the Phase 4 reusable component library, complete Phase 5 Patch editor, component-polished Phase 6 Mixer composition, browser workflows, multi-select, or final Figma polish"
		...
		later_roadmap_phases: "This increment does not introduce Phase 3 expansion beyond at most three effect slots per Patch and eight bus returns, Phase 4 components, the complete Phase 5 Patch editor or component-polished Phase 6 Mixer, asset workflows, Modal or MultiSelect behavior, modulation, arbitrary graph editing, persistence, or final visual completion"
	}

	completion: {
		requiredGoals: ["goal.play_test_song", "goal.control_synth", "goal.inspect_patch", "goal.select_patch_engine", "goal.edit_patch_envelope", "goal.select_soundfont_preset", "goal.shape_patch_with_effect", "goal.observe_synth", "goal.observe_live_synth", "goal.use_graphical_shell", "goal.use_semantic_graphical_view_model", "goal.route_patches_through_sixteen_tracks"]
		projectChecks: ["validation.format", "validation.clippy", "validation.test", "validation.smoke", "validation.capability_schema", "validation.static_patch_effect", "validation.patch_page_projection", "validation.engine_selection_workflow", "validation.soundfont_preset_selection", "validation.prepared_engine_rack", "validation.braids_engine", "validation.per_voice_envelope", "validation.control_dispatch_performance", "validation.demo_scene", "validation.schema_surface", "validation.egui_context", "validation.graphical_application_shell", "validation.semantic_graphical_view_model", "validation.sixteen_track_mixer_routing", "validation.mutation_harness", "validation.live_demo", "validation.production_runtime_contracts", "validation.audio_renderer_realtime_contract", "validation.prepared_graph_handoff_contract", "validation.audio_observation_realtime_contract", "validation.zero_selection_guard"]
	}
}
