...
project: {
	...
	layerRules: {
		...
		avoid: [
			...
			"effects other than the one statically configured Patch-local Chorus plus the one global reverb and one global delay; effect selection, bypass, multiple slots, reordering, or arbitrary routing",
			"sequencer, transport, timeline, pattern, clip, or song-editing domain models",
			"preset/session persistence or browsing, alternate SoundFont assets, modulation matrices, additional per-channel inserts, dynamic effect chains, arbitrary buses, or plugin hosting; the fixed Chorus insert and embedded SoundFont preset selection are permitted",
			"Phase 4 reusable component library, the complete Phase 5 Patch editor or component-polished Phase 6 Mixer, Modal or MultiSelect behavior before their owning workflows, asset browsers, final visual polish, or UI-owned domain/navigation/audio state; the Phase 1 shell, Phase 2 semantic model, corrective sixteen-track routing controls/meters, and retained diagnostic content are permitted",
		]
	}
	...
	validations: {
		...
		production_runtime_contracts: {
			...
			capabilities: ["capability.instrument_capability_model", "capability.static_patch_effect", "capability.asynchronous_engine_selection", "capability.prepared_engine_rack", "capability.realtime_execution"]
			goals: ["goal.play_test_song", "goal.control_synth", "goal.select_patch_engine", "goal.shape_patch_with_effect"]
			description: "the production constructor, exact instrument/effect registration, injected capacity-one preparation worker, negotiated complete-graph preparation, callback adaptation, control-owned shutdown, visible device failure, and routing observation contracts pass together"
		}
		...
		audio_observation_realtime_contract: {
			...
			capabilities: [
				...
				"capability.realtime_execution",
					...
					goals: ["goal.play_test_song", "goal.control_synth", "goal.select_soundfont_preset", "goal.shape_patch_with_effect"]
						description: "one fixed SoundFont parse supplies exact catalog choices and numeric prepared data while the MIDI fixture alternates real SoundFont/Braids engines and processes one first-Patch Chorus through the control, effect, mixer, and audio path"
			}
			prepared_engine_rack: {
				...
				capabilities: ["capability.prepared_engine_rack", "capability.static_patch_effect", "capability.realtime_execution", "capability.soundfont_audio", "capability.braids_engine", "capability.per_voice_envelope"]
				goals: ["goal.play_test_song", "goal.control_synth", "goal.shape_patch_with_effect"]
				description: "the named rack target proves bounded SoundFont/Braids dispatch followed by the aligned prepared Chorus rack, matching scalar/envelope/effect projection, complete-graph replacement, pressure recovery, and off-callback destruction"
			}
			braids_engine: {
			...
			per_voice_envelope: {
				...
				description: "all four ADSR values edit through the canonical PATCH reducer path without becoming track state, project exactly, and independently shape overlapping SoundFont and Braids note voices with bounded callback work"
			}
				capability_schema: {
				...
				resources: [
					...
					capabilities: ["capability.instrument_capability_model", "capability.static_patch_effect", "capability.one_way_parameter_control", "capability.soundfont_audio", "capability.soundfont_preset_selection"]
					goals: ["goal.control_synth", "goal.select_soundfont_preset", "goal.shape_patch_with_effect"]
						description: "both instrument descriptors, the separate Chorus descriptor, catalog-hydrated SoundFont preset Choice, and generic fixture Patch/effect configs survive reducer installation, serialization, and projection while malformed, unknown, or mismatched configs fail without fallback or bypass"
				}
				patch_page_projection: {
					...
					resources: [
						...
						capabilities: ["capability.schema_driven_patch_page", "capability.static_patch_effect", "capability.asynchronous_engine_selection", "capability.instrument_capability_model", "capability.one_way_parameter_control", "capability.per_voice_envelope", "capability.soundfont_preset_selection"]
						goals: ["goal.inspect_patch", "goal.select_patch_engine", "goal.edit_patch_envelope", "goal.select_soundfont_preset", "goal.shape_patch_with_effect"]
						description: "direct 1/2 input, reducer-owned Patch and dynamic Engine-plus-ADSR-plus-instrument-StructuralChoice-plus-effect-ScalarEdit focus, exact SoundFont/Braids/Chorus rows and lifecycle status, preserved MIXER projection, audio-neutral navigation, canonical ADSR/effect editing, and semantic engine/preset request projection pass through production seams"
				}
				...
				capabilities: [
					...
					goals: ["goal.select_patch_engine", "goal.edit_patch_envelope", "goal.select_soundfont_preset", "goal.shape_patch_with_effect", "goal.observe_synth"]
					description: "the exhaustive deterministic GUI scene covers every current event, focused PATCH ADSR/Chorus/catalog-backed preset control, editable scalar and structural controls, effect order/isolation/independent state, coexistence, lifecycle state/effect/failure, serialized property, projection, and downstream audio consequence"
			}
			live_demo: {
				...
				capabilities: [
					...
					goals: ["goal.edit_patch_envelope", "goal.select_soundfont_preset", "goal.shape_patch_with_effect", "goal.observe_live_synth"]
					description: "a deterministic-clock harness proves live pacing, schedule-independent semantic Patch probes around every scalar checkpoint, the focused Patch's four ADSR plus Chorus Amount/Depth instances through PATCH, exact scalar coverage, effect-config preservation across the adjacent preset and both engine replacements, coherent lifecycle/revision/effect-audio checkpoints, semantic all-notes-off completion, mapped-input isolation, controlled no-progress timeout cleanup, one completed report, and immediate successful close without substituting a second reducer, worker workflow, or renderer"
			}
			schema_surface: {
				...
				capabilities: ["capability.instrument_capability_model", "capability.static_patch_effect", "capability.schema_driven_patch_page", "capability.asynchronous_engine_selection", "capability.observable_demo_scene"]
				goals: ["goal.control_synth", "goal.inspect_patch", "goal.select_patch_engine", "goal.shape_patch_with_effect", "goal.observe_synth"]
				description: "typed instrument/effect production surface descriptors and discovered serialized leaves are exactly equal with no missing or unexpected identifiers"
			}
			egui_context: {
				...
				description: "a headless egui Context dispatches real RawInput through EframeGraphicalApplication.update into AppLoop and proves the next frame, event record, accepted state, exact shell and diagnostic projection values, structural regions, engine-row lifecycle status, selection, and scroll target all reflect that event"
			}
			mutation_harness: {
				...
				description: "six isolated real-seam mutants cover dropped edits, cross-track parameter leakage, MIDI misrouting, schema omission, dry-to-wet bypass, and zero rendering with schema-valid measured counterexamples"
			}
		}
	...
	invariants: core: [
		...
		{text: "input and view adapters emit AppEvents and never mutate application or engine state", meta: rationale: "adapters remain replaceable"},
			{text: "PATCH and MIXER are the only top-level contexts; InteractionState owns context and stable PATCH focus resolved as Engine, canonical ADSR, instrument StructuralChoice controls, then ordered effect ScalarEdit controls, while the graphical window renders only immutable GraphicalShellProjection data and the retained TextProjection remains diagnostic", meta: rationale: "page selection remains semantic, deterministic, capability-driven, and independent from UI state"},
		...
		{text: "the only Patch-local processor is one capability-described Chorus on the configured first Patch, ordered between synthesis and MixEngine; the mixer still owns only one global reverb and one global delay", meta: rationale: "the first effect proves extensible ownership with a fixed bounded topology"},
		{text: "AppState owns exactly sixteen persistent MixerTracks; each Patch owns only one validated output track and trim, MixEngine sums post-effect Patch stems into fixed track scratch before applying track level/pan/mute/solo/sends, and meters observe tracks before the audibility gate", meta: rationale: "Patch identity and mixer-channel identity are distinct product concepts"},
