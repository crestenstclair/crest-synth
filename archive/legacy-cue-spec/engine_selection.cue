...
project: contexts: Control: {
	valueObjects: PatchControlId: {
		...
		from: "Engine | Envelope(VoiceEnvelopeParameter) | Capability(ParameterId) | Effect(EffectSlotId, ParameterId) | Output(PatchOutputParameter)"
		invariants: [
			...
			"PatchUtility has a distinct ordered resolver containing Output(TrimGain) then Output(OutputTrack); those controls reuse the canonical PatchOutput descriptor and never enter the PatchMain order",
			"Engine serializes as patch.engine, Envelope(parameter) derives patch.envelope.<VoiceEnvelopeParameter.name>, Capability(parameter) derives patch.capability.<ParameterId>, Effect(slotId, parameter) derives patch.effect.<EffectSlotId>.<ParameterId>, and Output(parameter) derives patch.output.<PatchOutputParameter.name> without declaring a second domain-field enum",
		...
		contributesTo: [
			...
			{capability: "capability.sixteen_track_mixer_routing", contribution: "addresses canonical Patch trim and output track in PATCH Utility"},
		]
	}
...
project: contexts: RealTime: {
	valueObjects: GraphPreparationRequest: {
		...
		invariants: [
			"candidatePatches has the exact active PatchIds, order, capacity, PatchOutput values, envelopes, and ordered PostEffectConfigs and candidateParameters has the exact complete MixerState, with only patchId's InstrumentConfig replaced by the intent's descriptor-default capability config or one-assignment structural-choice config",
