...
The CUE files in `spec/` are executable implementation declarations. Reconcile them to `DESIGN.md` before implementation. A deliberately narrow implementation slice must not redefine the product. Use OpenSpec with the evaluated CUE architecture as pivotal planning and implementation context; do not invoke legacy crest-spec tooling.
