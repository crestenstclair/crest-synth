...
- 2026-08-02T17:05:00Z – review – shell_pid=8651 – Reviewed and rejected (review-cycle-1.md). Verified by running the release binary: walked digits 1-8, pressed the unbound 9, closed the window; all 15 witness.component_gallery predicates satisfied, make test exit 0, clippy 0 warnings, fmt clean, ownership exact, --demo-live still the autonomous witness. Three issues found. (1) The window minimum was the Desktop authored viewport (1920x1080); measured on a 1920x1080 display the frame was 1920x1112 at (0,30), putting 62 px — the footer band carrying the only on-screen browsing affordance — below the screen edge, with the pinned minimum removing the operator's remedy. The paint pass reported 0 clipped text because it measures against the egui surface, not the display. (2) NFR-005's "at both authored sizes" clause was unmeasured: states_painted was a flat array while bands_painted was already per-policy, so nine states painted in one column alone satisfied the count. (3) Page 6 specimens were not labelled with their state name, against T029 step 5.
- 2026-08-02T17:32:00Z – implement-command – shell_pid=23872 – Fixed all three review-cycle-1 issues in src/testing/component_gallery_scene.rs. The window minimum is now the authored compact width by a measured 856 px height, bound at both ends by test (856 composes clean across all eight pages in both columns; 848 clips page 5's vertical separator) and held against the authored desktop display at compile time via a declared 96 px window-chrome allowance, measured at 62 px on macOS. State coverage is now indexed by density policy: a state counts only when every policy painted it, each composition is judged for colourless distinctness against itself, the observation names the compositions each state reached, and a test proves the count reports a shortfall when one composition's record is removed. Page 6 rows now lead with the state name (Loading · ENGINE), with a test that the label a reader sees names its state. Re-verified by running the real window at the new size: geometry 1280x888 at (320,71) — wholly on screen with the footer band visible — walked digits 1-8, pressed 9, closed; all 15 predicates satisfied, exit 0, states_rendered names Desktop and SteamDeck for all nine states. 24 unit tests (was 21), make test exit 0, clippy 0 warnings, fmt-check clean.
- 2026-08-02T17:55:00Z – review – shell_pid=53304 – Reviewed and approved (cycle 2). Verified by running the window, not by reading the diff. All three review-cycle-1 issues are fixed and each fix is measured: (1) the window opened at 1280x888 positioned (320,71) on a 1920x1080 logical display — bottom edge 959, wholly on screen, footer band visible in the screenshot; the compile-time bound was falsified by raising MINIMUM_GALLERY_HEIGHT_PX to 1000, which fails compilation with the declared message, and the lower bound is held by the 848-clips test. (2) states_painted is now indexed by density policy; states_rendered named Desktop and SteamDeck for all nine states, and the shortfall test proves the count drops to 8 when one composition's record is removed. (3) Page 6 rows read "Loading · ENGINE", confirmed on screen. Live walk of digits 1-8 then the unbound 9: page 9 and page 8 screenshots are byte-identical (md5 567da1b1...), page 7 differs, so retention is measured rather than asserted. The emitted ComponentGalleryObservation satisfied all fifteen witness.component_gallery predicates as declared in .kittify/crest-spec/proof/witnesses.yaml, app_state_generation_delta 0 across the walk. 24 gallery unit tests, make test exit 0 (587 lib plus every integration target, 0 failures), clippy --all-targets --all-features 0 warnings, cargo fmt --check clean. make demo-live still runs the autonomous effects-and-buses witness: exit 0, 144 checkpoints, 105/105 editable parameters, 0 dropped, cleanup=true — the gallery never became an alias. Ownership exact: the WP commits touch only the four declared owned_files and no other WP declares any of them. Non-blocking: the doc comment on paint_composition says the page is recorded painted "after both compositions did", but record_painted_page fires inside each composition, so the first one to emit runs marks it — the doc overstates what the code does; the counter itself is honest.
