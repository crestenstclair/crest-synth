...
project: assets: StandaloneUiMain: {
	...
	prompts: [
		...
		#"RENDER THE MIXER VIEW: draw the 6 currently-visible channel strips (the window MixerView exposes via its viewportOffset) side by side. Each strip is vertical with these rows top-to-bottom: Volume, Reverb send, Echo send, Pan, Mute, Solo. The VOLUME control IS the level strip: a vertical strip, dark at rest, that animates to show that channel's live peak level (read each channel's PeakLevel from the ChannelMixer) — it both sets volume and meters. Every channel meters independently and metering is UNAFFECTED by solo (a channel silenced by another's solo still shows its own level). Highlight the cursor's (channel, parameter) cell. While Edit (J) is held, the cursor changes color, and when the focused row is Volume the full box containing the level strip is highlighted. Show Mute/Solo as toggle indicators."#,
