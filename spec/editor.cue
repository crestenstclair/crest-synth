...
project: invariants: standaloneEditor: [
	...
	{text: "--autopilot is a real end-to-end run that opens the actual window/audio device, injects notes through the live engine, and asserts in code that device-bound audio is non-silent and all sixteen T00-T0F track columns plus the selected-track inspector are laid out in the initial view before self-terminating", meta: rationale: "the validation must fail when the app is silent, omits tracks, or loses the inspector; a completion token alone is theater"},
...
project: invariants: designSystem: [
	...
	{text: "a skin reads colors only by resolving a SemanticToken through the Theme port; layout constants describe the terminal grid centrally and draw code contains no ad-hoc literal colors or per-cell sizing", meta: rationale: "one theme and one layout vocabulary keep the dense grid coherent and restylable"},
