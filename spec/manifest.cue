...
project: assets: BehavioralAcceptanceTests: {
	...
	prompts: [
		...
		"capability_schema constructs both production providers and CapabilityRegistry, installs discriminating generic Patch configs through AppState, and asserts exact descriptor/config serialization plus generic text projection; it must also prove unknown, duplicate, missing, undeclared, wrong-kind, non-finite, and out-of-range mutations fail without fallback. patch_page_projection drives Digit1/Digit2 through the production translator and AppLoop, proves reducer-owned Patch/engine focus, exact SoundFont and Braids rows, Ready/Preparing status, preserved MIXER selection/body, audio-neutral context switching, semantic K+A/D request projection, typed unsupported PATCH rejection, and sample-identical rendering before a candidate activates. braids_engine proves source pins/license, exact-rate preparation, all 47 models, one sixteen-voice bank per Braids Patch, 16 × N scaling, Patch-local stealing, scalar isolation, finite audio, FFI lifecycle, and measured timing. per_voice_envelope proves exact projection and independently releasing overlapping SoundFont and Braids notes while SoundFont retains one synthesizer per Patch. exhaustive_demo_scene asserts bidirectionally exact input/event/context/state/projection/audio and engine-lifecycle coverage, every typed scalar boundary, both successful engine directions, controlled failure, and the declared descriptor-default SoundFont final state; schema_surface asserts bidirectional equality between production-owned typed descriptors and observed serialized leaves including both installed schemas, both contexts, and every engine-selection status/failure/effect.",
		"live_demo_scene uses a deterministic monotonic clock, a frame-observation harness, the production worker/structural ports, and the production renderer plus AtomicAudioObservation to prove pacing, one accepted change and audible generation-tagged observation for every frozen editable parameter instance, then SoundFont-to-Braids-to-descriptor-default-SoundFont with exact Preparing/Activating/Ready status, increasing revisions, targeted finite nonzero output, exact scalar and transition coverage, rejection recovery, semantic all-notes-off, and an inert completed runner. It must not duplicate AppState::apply, worker orchestration, graph handoff, projection, render, coverage, or report logic, and it must not open or skip based on a native window or physical device.",
...
project: assets: CrestSynthMain: {
	...
	prompts: [
		"Create src/bin/crest_synth.rs as composition only. Construct HiDefSoundFontCapability, BraidsCapability, their separate preparers, ThreadedGraphPreparationWorker, and distinct audio, structural, and observation boundaries; inject all ports into StandaloneApplication, which validates and freezes the provider descriptors, and call run.",
		...
		"Normal invocation expects ./sf2/HiDef.sf2, the vendored pinned Braids DSP, and ./midi/Corridors of Time - Chrono Trigger.mid, negotiates the physical device without starting it, builds the complete alternating initial graph for that exact accepted configuration before starting the stream, and begins MIDI only after that succeeds. The PATCH engine row uses the injected capacity-one preparation worker; there is no play command, transport, file chooser, engine-choice modal, or alternate effect selection.",
		...
		"When --demo-scene is present, print exactly one single-line CREST_EVENT_LOG JSON object, one single-line CREST_STATE_TREE JSON object, and one single-line CREST_OBSERVATION JSON object. Use stable Serialize data, never Debug output or Markdown; the ordinary interactive run emits no trace unless explicitly requested.",
			...
			"After frozen scalar coverage, --demo-live selects the focused first Patch SoundFont to Braids and back to descriptor-default SoundFont through semantic AppEvents and ThreadedGraphPreparationWorker; wait nonblockingly for each Preparing, Activating, Ready sequence and newer acknowledged graph revision, dispatch targeted MIDI, and require finite nonzero target output before advancing or reporting completion.",
