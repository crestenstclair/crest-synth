...
project: assets: RootCargoToml: {
	...
	prompts: [
		...
		"Package name crest-synth, edition 2021, a lib target plus binary targets under src/bin/. The lib target additionally declares `crate-type = [\"cdylib\", \"rlib\"]` so it can also be loaded as a CLAP/VST3 plugin binary by a host — the cdylib is what the plugin framework needs to produce a loadable artifact; rlib keeps the existing binaries linking against the crate as before.",
		"Dependencies and why each exists: cpal (audio output), midir (MIDI input), eframe + egui (GUI), gilrs (gamepad input), rtrb (lock-free SPSC ring buffer), triple_buffer (lock-free latest-wins parameter sharing), basedrop (deferred deallocation for real-time), serde + serde_json (preset/session serialization), symphonia (audio file decoding), midly (Standard MIDI File parsing), nih-plug (CLAP/VST3 plugin framework — the Plugin bounded context's PluginHost port is implemented by the NihPlugHost adapter over this crate; pull it from the nih-plug git repository per its own README, as it is not published to crates.io).",
