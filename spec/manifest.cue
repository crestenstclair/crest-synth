...
project: assets: BuildMakefile: {
	...
	prompts: [
		...
		"Keep demo exactly headless and deterministic. demo-live is the only autonomous target that opens the real window and physical audio device; after scene completion it emits final evidence once, closes the window, releases the stream, and returns success.",
...
project: assets: BehavioralAcceptanceTests: {
	...
	prompts: [
		...
		"capability_schema constructs both production providers and CapabilityRegistry, installs discriminating generic Patch configs through AppState, and asserts exact descriptor/config serialization plus generic text projection; it must also prove unknown, duplicate, missing, undeclared, wrong-kind, non-finite, and out-of-range mutations fail without fallback. patch_page_projection drives Digit1/Digit2 through the production translator and AppLoop, proves reducer-owned context and stable PatchId focus, exact SoundFont and Braids PatchPageProjection rows and engine choices, unchanged session/parameter values and graph revision, no audio or structural command, preserved MIXER selection/body, typed PATCH navigation/adjustment rejection, successful later page selection, and sample-identical production rendering from identical prepared state. braids_engine proves source pins/license, exact-rate preparation, all 47 models, one sixteen-voice bank per Braids Patch, 16 × N scaling, Patch-local stealing, scalar isolation, finite audio, FFI lifecycle, and measured timing. per_voice_envelope proves exact projection and independently releasing overlapping SoundFont and Braids notes while SoundFont retains one synthesizer per Patch. exhaustive_demo_scene asserts bidirectionally exact input/event/context/state/projection/audio coverage, every typed parameter boundary, and exact baseline restoration; schema_surface asserts bidirectional equality between production-owned typed descriptors and observed serialized leaves including both installed schemas and both context projections.",
...
project: assets: ProductionRuntimeContractTests: {
	...
	prompts: [
		"Use the public production constructor with injected provider, preparer, structural, observation, device, and window fixtures; prove matching registration and typed duplicate, missing, unknown, and mismatched rejection before structural publication.",
...
project: assets: CrestSynthMain: {
	...
	prompts: [
		"Create src/bin/crest_synth.rs as composition only. Construct HiDefSoundFontCapability, BraidsCapability, and their separate preparers plus distinct audio, structural, and observation boundaries; inject all ports into StandaloneApplication, which validates and freezes the provider descriptors, and call run.",
