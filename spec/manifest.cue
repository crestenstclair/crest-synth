...
project: assets: BuildMakefile: {
	...
	prompts: [
		...
		"Provide these Cargo-backed targets: build (cargo build), check (cargo check --all-targets), test (cargo test --all-targets), lint (cargo clippy --all-targets -- -D warnings), fmt (cargo fmt --all), fmt-check (cargo fmt --all -- --check), run (cargo run --bin crest-synth), smoke (cargo run --bin crest-synth -- --smoke), observe (cargo run --bin crest-synth -- --smoke --observe), demo (cargo run --bin crest-synth -- --smoke --observe --demo-scene), and clean (cargo clean).",
...
project: assets: BehavioralAcceptanceTests: {
	...
	prompts: [
		"Create four explicit Cargo integration-test targets: tests/exhaustive_demo_scene.rs, tests/schema_surface.rs, tests/eframe_context.rs, and tests/behavioral_mutation_harness.rs. Project validations invoke them with cargo test --test, so a missing target is a hard failure.",
	...
	contributesTo: [{capability: "capability.observable_demo_scene", contribution: "makes every new acceptance gate structurally executable and impossible to satisfy with zero matched tests"}]
}
