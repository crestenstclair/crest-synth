...
// Override (phase 7): adds `demo-effects` (renders the multi-patch demo through
// per-patch + global EffectChains, proving slot-order and bypass — validation-
// safe) and `play-effects` (human-only afplay). Picked up by run-phased-agent.sh
// (highest-numbered phase-N.override-BuildMakefile.cue with N <= target phase).
// REPLACES the phase-6 override, so the FULL cumulative target list is here.
//
...
project: assets: BuildMakefile: {
	...
	prompts: [
		...
		"Use cargo for all Rust operations. Declare .PHONY for all targets. afplay must appear ONLY in play-midi, play-voices, play-tone, play-patches, play-mod, play-samples, and play-effects.",
	]
}
