...
// Override (phase 6): adds `demo-samples` (renders the hermetic SampleLibrary
// prover to WAV — validation-safe) and `play-samples` (human-only afplay).
// Picked up by run-phased-agent.sh (highest-numbered phase-N.override-
// BuildMakefile.cue with N <= target phase). REPLACES the phase-5 override, so
...
project: assets: BuildMakefile: {
	...
	prompts: [
		...
		"demo-samples: cargo run --bin sample_demo  — synthesizes a sample, loads it, resolves key/velocity zones, interpolates, renders to sample-demo.wav and prints `zones loaded=` / `zone hit:` markers. Takes no FILE argument; fully hermetic.",
		...
		"Use cargo for all Rust operations. Declare .PHONY for all targets. afplay must appear ONLY in play-midi, play-voices, play-tone, play-patches, play-mod, and play-samples.",
	]
}
