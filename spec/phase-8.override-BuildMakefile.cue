...
// Override (phase 8): adds `demo-presets` (serializes a full Setup, reloads it,
// proves bit-identical re-render — validation-safe) and `play-presets` (human-
// only afplay). Picked up by run-phased-agent.sh (highest-numbered phase-N.
// override-BuildMakefile.cue with N <= target phase). REPLACES the phase-7
...
project: assets: BuildMakefile: {
	...
	prompts: [
		...
		"demo-presets: cargo run --bin preset_demo  — serializes a full Setup, reloads it, proves the reloaded Setup re-renders bit-identical audio, writes preset-demo.wav and prints `setup roundtrip: equal` / `render identical: true`. Takes no FILE argument.",
		...
		"Use cargo for all Rust operations. Declare .PHONY for all targets. afplay must appear ONLY in play-midi, play-voices, play-tone, play-patches, play-mod, play-samples, play-effects, and play-presets.",
	]
}
