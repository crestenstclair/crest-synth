...
fn poll_for_period(
    ...
    let res = match alsa::poll::poll(descriptors, *poll_timeout) {
        Ok(res) => res,
        // A handled signal interrupts the wait, not the device. Return to the
        // worker loop so it also observes a concurrent stream shutdown.
        // alsa 0.11's poll wrapper returns negative errno (unlike its PCM API).
        Err(error) if error.errno().unsigned_abs() == libc::EINTR as u32 => {
